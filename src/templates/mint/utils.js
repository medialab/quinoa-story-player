...
const findChildren = (item, arr) => {
  const children = arr.filter((thatItem) => thatItem.parentId === item.id);
  if (children && children.length) {
    return {
      ...item,
      children: children.map((item2) => findChildren(item2, arr))
    };
  }
  return {
    ...item
...
export const nestSummary = (toc) => {
  const summary = toc.reduce((res, block, blockIndex) => {
    let parentId;
    if (blockIndex > 0) {
      const prev = res[blockIndex - 1];
  ...
  return summary.reduce((res, item) => {
    if (!item.parentId) {
      const newItem = findChildren(item, summary);
      return [...res, newItem];
    }
    return res;
  }, []);
